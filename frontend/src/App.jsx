...
// still being played) don't follow the "WC\d{4}-XXX-XXX" three-letter
// pattern decodeMatchId below expects — this covers those by matchId
// directly, once the real matchup is known. Doesn't touch statAKey/
// predicate/question at all; those still come from the real on-chain
// data via the auto-discovery path, this only fixes the display names.
const MATCH_ID_OVERRIDES = {
  "WC2026-FINAL": { teamA: "Spain", teamB: "Argentina" },
};

function decodeMatchId(matchId) {
  if (MATCH_ID_OVERRIDES[matchId]) return MATCH_ID_OVERRIDES[matchId];
...
export default function App() {
  ...
  const attemptSettlement = useCallback(async (market) => {
    ...
    // Self-healing instead of a dead end: earliestSettleTs is a safety
    // margin chosen when the market was created — a guess, unavoidably,
    // since nobody knows in advance whether a knockout match will need
    // extra time. Rather than surface that guess-being-wrong as a failure
    // and send someone off to manually run a Playground script with a
    // hand-picked timestamp, this corrects it right here: if TxLINE's
    // latest available data for this fixture is still older than the
    // configured earliestSettleTs, push earliestSettleTs back to exactly
    // that latest available timestamp (never forward — set_earliest_settle_ts
    // only allows moving it earlier) and continue. Requires the connected
    // wallet to be this market's authority; anyone else attempting
    // settlement on a not-yet-correctable market gets a clear error
    // instead, same as before.
    if (market.earliestSettleTs && loBody.ts < market.earliestSettleTs * 1000) {
      const correctedTs = Math.floor(loBody.ts / 1000);
      console.log(
        `[attemptSettlement] earliestSettleTs (${new Date(market.earliestSettleTs * 1000).toISOString()}) ` +
        `is later than TxLINE's latest available data (${new Date(loBody.ts).toISOString()}) — ` +
        `attempting to auto-correct it to the real timestamp instead of failing.`
      );
      try {
        const fixSig = await program.methods
          .setEarliestSettleTs(new BN(correctedTs))
          .accounts({ authority: wallet.publicKey, market: marketPda })
          .rpc();
        console.log(`[attemptSettlement] earliestSettleTs corrected on-chain. TX: ${fixSig}`);
      } catch (fixErr) {
        throw new Error(
          `TxLINE's latest available data for this fixture (seq=${seq}) is dated ` +
          `${new Date(loBody.ts).toISOString()}, before this market's earliestSettleTs ` +
          `(${new Date(market.earliestSettleTs * 1000).toISOString()}). Tried to auto-correct it, ` +
          `but that failed — likely because your connected wallet isn't this market's authority. ` +
