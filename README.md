...
ChampChain lets users open binary prediction markets on World Cup matches
...
A real market is live on devnet:

| Field | Value |
|---|---|
| Match | Argentina vs Croatia |
| Market PDA | `7qa3vReBAayekuJGAHUfzzZRAmWiBxxNq6jCd898t8Wa` |
| Question | Will Argentina score more than 1.5 goals? |
| Created TX | `37VxfA5bbM4GrD5KLWPRfsHcUMPzJ17qEVJZgqTT1EAo4J9xasYFhcNXAE2uUnmorQ32q3Hu2q6sfaoxJV5yBxhj` |
...
Every TxLINE HTTP endpoint (`/api/scores/*`) requires
`Authorization: Bearer <jwt>` + `X-Api-Token: <apiToken>`. The frontend gets
both through the exact flow documented at
`https://txline.txodds.com/documentation/worldcup`, wired end-to-end:

1. Connect a devnet wallet, click **"Activate TxLINE access"** in the header.
2. `subscribeOnChain()` sends a real `subscribe(service_level_id=1, weeks=4)`
   transaction to the TxLINE program, built from the actual fetched devnet
...
"SIMULATED — ACTIVATE TXLINE" feed to the real authenticated
`GET /api/scores/stream` (via `fetch` + `ReadableStream`, since `EventSource`
cannot send custom headers), and `attemptSettlement()` can call
`GET /api/scores/stat-validation` with the required headers. See
`frontend/src/lib/txlineAuth.js`, `txlineStream.js`, `txlineConfig.js`, and
`frontend/src/hooks/useTxlineAuth.js`.

## Security highlights

Sixteen attack vectors are documented in `SECURITY_NOTES.md`. Key points:
...
- **Checked arithmetic throughout** — `checked_add`, `checked_mul`, `u128`
  intermediate for payout calculation.
...
- [x] Real market created on-chain (Argentina vs Croatia)
...
Cup guide doesn't show argument widths, and (2) nothing on the streaming
page warns that plain `EventSource` silently can't carry the required auth
headers — both were only resolved by pulling the actual devnet IDL from
`/documentation/programs/devnet` and reading the raw SSE bytes by hand.
